...
### Transaction
- Setiap insert/update Transaction wajib memicu rekalkulasi `cached_balance` Wallet terkait, **dalam satu database transaction** yang sama (08. System Design, 8.3) — tidak ada jalur kode yang menulis Transaction tanpa melalui fungsi ini.
...
### Forecast
- Endpoint Forecast **tidak pernah menghitung ulang secara sinkron** saat dipanggil biasa — dia membaca `forecast_snapshots` terakhir yang sudah dihasilkan cron job (08. System Design, 8.3). Hanya ada satu jalur resmi untuk trigger recompute: cron terjadwal.
...
- Endpoint AI Copilot Cashflow Prediction **wajib** memanggil `forecast.service.ts` untuk mengambil snapshot yang ada, tidak diizinkan menghitung proyeksi sendiri di dalam `ai-copilot.service.ts` (08. System Design, 8.3 — poin paling kritis di seluruh sistem).
...
- Tipe data sesuai kolom database (07. Database) — terutama `numeric` untuk field uang, jangan menerima string yang belum diparse
