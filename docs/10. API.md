...
**Cold-start response:** kalau `context_builder` mendeteksi data tidak cukup (belum ada transaksi sama sekali), `reply` berisi pengakuan eksplisit keterbatasan data (Edge Case Section 4.8) — ini logic di `ai-copilot.service.ts`, bukan sekadar berharap LLM (Gemini 2.5 Flash) menjawab begitu tanpa instruksi eksplisit di prompt.
