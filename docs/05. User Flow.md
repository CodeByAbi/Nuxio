...
**Status dokumen:** Melengkapi 04. PRD (ringkasan eksekutif) dan merinci Section 5 (User Flow) serta Section 6 (Information Architecture) dari MVP Product Blueprint. Diagram alur ada di percakapan terkait; dokumen ini berisi penjelasan, keputusan desain, dan edge case yang menyertai setiap tahap.

---

## 5.1 Alur Utama: Registrasi sampai Calendar
...
| User skip Budget dan Tagihan Rutin, lalu langsung lihat Calendar kosong                 | Calendar tetap tampil penuh (grid bulan), dengan catatan bahwa Forecast baru mencakup income yang sudah diisi (11. Frontend Specification, 11.5 Empty States) |

---

## 5.2 Alur Interaksi Harian: Calendar sebagai Pusat
...
| Tambah Transaction     | Ya, lewat `DayDetailPanel` saat tanggal diklik                      | Halaman Transaction — **wajib pakai komponen form yang sama** (11. Frontend Specification, 11.2), supaya data konsisten dari dua entry point |
...
| Lihat Forecast         | Overlay garis proyeksi langsung di grid Calendar                    | Tidak ada halaman terpisah — ini keputusan sengaja (lihat 5.3)                                                                               |
...
## 5.3 Kenapa Forecast Tidak Jadi Halaman Terpisah
...
Prinsip yang sama berlaku untuk **Budget** dan **Goal**: keduanya tetap punya halaman manajemen sendiri untuk operasi CRUD yang butuh detail (menambah/mengubah/menghapus), tapi _representasi_ dan _insight_ utamanya selalu dikembalikan ke Calendar lewat indikator dan overlay — bukan halaman laporan yang berdiri sendiri.

---

## 5.4 Information Architecture (Ringkasan Navigasi)
...
├─ Budget
├─ Goal
├─ Forecast              (opsional sebagai item sidebar — cukup toggle di Calendar, lihat 5.3)
...
## 5.5 Ringkasan Ketergantungan Alur

Beberapa bagian alur tidak bisa berjalan independen — ini menentukan urutan pengujian end-to-end nantinya:

- **Calendar** adalah _consumer_ data dari Wallet, Transaction, Budget, Goal — dia tidak menghasilkan data sendiri. Kalau salah satu dari keempatnya kosong/error, Calendar tetap harus tampil (dengan empty state yang sesuai), bukan ikut gagal.
...
- **AI Copilot Cashflow Prediction** bergantung penuh pada Forecast — dia tidak pernah menghasilkan proyeksi dari nol (lihat juga 08. System Design, Section 10 untuk alur teknisnya).
...
| Dokumen                    | Isi                                                                                   |
| -------------------------- | ------------------------------------------------------------------------------------- |
| 04. PRD                    | Ringkasan eksekutif, scope, hipotesis, metrik                                         |
| 11. Frontend Specification | Komponen `DayDetailPanel`, `TransactionForm` yang dipakai lintas entry point          |
| 12. UI Design System       | Pola visual empty state, warna semantik untuk indikator Calendar                      |
| 08. System Design          | Request lifecycle teknis untuk Transaction dan AI Copilot yang disebut di dokumen ini |
