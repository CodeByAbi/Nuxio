...
| Untuk                              | Masalah                                                                          | Nuxio memberikan                                                                                  |
| ---------------------------------- | -------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- |
...
Kebanyakan aplikasi keuangan personal (expense tracker, budget app konvensional) bersifat **retrospektif**: mereka baik dalam mencatat apa yang sudah dibelanjakan, tapi lemah dalam menjawab pertanyaan yang sebenarnya paling sering muncul di kepala user — _"Apakah uang saya cukup sampai akhir bulan?"_, _"Kalau saya tambah pengeluaran ini, budget saya masih aman tidak?"_, _"Apakah goal saya realistis dengan pemasukan saya sekarang?"_
...
Nuxio secara spesifik menempatkan proyeksi masa depan (Forecast) sebagai bagian dari pengalaman kalender sehari-hari, dan menempatkan AI sebagai lapisan penjelas di atas perhitungan domain yang deterministik — bukan pengganti perhitungan itu sendiri.
...
2. **Calendar adalah pusat aplikasi.** Semua modul lain (Wallet, Budget, Goal, Forecast) adalah _sumber data_ yang diproyeksikan ke Calendar — bukan halaman yang berdiri sendiri dan terpisah dari alur utama.
...
**User Story:** Sebagai user baru, saya ingin mendaftar dengan email dan password, agar saya bisa mulai menggunakan Nuxio.
...
| ------ | ---------------------------------------------- | -------------------------------------------------- | --------------------------------------------- |
| US-001 | user baru                                      | mendaftar dengan email dan password                | saya bisa mulai menggunakan Nuxio             |
...
| US-099 | user | yakin fitur inti saya tidak terganggu oleh status plan      | saya tetap bisa memakai Nuxio secara penuh selama MVP         |
...
**Purpose:** Halaman publik yang menjelaskan value proposition Nuxio dan mengarahkan visitor ke Register/Login.
...
- QA test grounding: ambil sample respons AI, verifikasi setiap angka yang disebutkan cocok dengan hasil query langsung ke domain service yang sama (lihat FR-124 dan Section 18 Testing Standards yang relevan — cross-reference dokumen arsitektur jika Nuxio kelak memiliki dokumen ADR terpisah).
...
> Catatan konsistensi: Nuxio **tidak memakai ORM** (Prisma atau lainnya — keputusan final `03. Architecture Decisions.md` Section 5). Akses database lewat Supabase client ber-token user (jalur request user, RLS aktif) dan Postgres function via RPC untuk operasi atomik — keduanya parameterized. Service role hanya untuk operasi privileged (background job, admin) dan **wajib** men-scope `workspace_id` manual (lihat Strategi Akses Database di atas).
