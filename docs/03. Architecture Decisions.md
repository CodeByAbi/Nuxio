...
| **Convention over Configuration** | Next.js App Router, Prisma, dan Zod semuanya punya konvensi kuat (file-based routing, schema-first) yang mengurangi keputusan berulang tim kecil. | Fleksibilitas struktur berkurang — tim harus mengikuti konvensi framework, bukan pola bebas custom. |
...
| **Strong Type Safety** | TypeScript end-to-end + Zod + Prisma memastikan kontrak data konsisten dari database sampai UI, krusial untuk domain finansial di mana tipe data yang salah (mis. float vs integer) adalah kelas bug paling mahal. | Waktu setup awal tipe/schema lebih lama dibanding kode dinamis tanpa tipe. |
...
- Setiap domain service mengekspos interface yang dipanggil AI Module — AI tidak pernah query Prisma langsung (Section 8).
...
- **Why:** Type-safe query builder yang menghasilkan tipe TypeScript langsung dari skema database — menyambungkan Strong Type Safety (Section 3) dari database sampai ke domain service. Migration tooling (`prisma migrate`) memberi riwayat migrasi yang terdokumentasi dan reversible (Definition of Done Section 9 blueprint, `07. Database.md` 7.4).
- **Alternatives considered:** Drizzle ORM (lebih ringan dan SQL-like, tapi ekosistem migration tooling dan Prisma Studio yang matang lebih menguntungkan tim kecil tanpa dedicated DBA), raw SQL query builder seperti Kysely tanpa ORM (kontrol penuh, tapi kehilangan auto-generated types dan migration history bawaan).
- **Trade-offs:** Query builder Prisma kurang optimal untuk agregasi SQL yang sangat kompleks — di titik ini raw query dipakai sebagai pengecualian terkontrol (`08. System Design.md` 8.1), bukan default.
- **Consequences:** Setiap perubahan skema wajib lewat `prisma migrate dev` di development dan `prisma migrate deploy` di staging/production — tidak ada perubahan skema manual di database manapun (`07. Database.md` 7.4 poin 1).
...
- **Consequences:** Build command menjalankan `prisma migrate deploy` otomatis sebelum `next build` (`14. DevOps.md` 14.6) — migration selalu berjalan sinkron dengan deployment, tidak pernah manual di production.
...
- **Trade-offs:** Skema validasi harus dijaga sinkron dengan tipe Prisma secara manual (tidak auto-generate dari skema database) — diterima sebagai biaya kecil demi kontrol eksplisit atas apa yang diterima dari client (mass assignment protection, `15. Security.md` 15.4).
...
**Consequences:** Review kode menolak PR apa pun yang memberi AI Module akses ke Prisma client langsung, atau ke domain service method yang bersifat mutasi (`create*`, `update*`, `delete*`).
...
**Context.** Jika AI Module diberi akses langsung ke Prisma/PostgreSQL, dua risiko muncul: (1) AI berpotensi menghitung sendiri angka yang seharusnya berasal dari domain service (melanggar Product Principle #3), dan (2) permukaan serangan bertambah — bug di query yang dipanggil AI Module bisa membuka celah kebocoran data lintas-Workspace tanpa melewati lapisan otorisasi domain yang sudah teruji.

**Decision.** AI Module **hanya** boleh memanggil method publik dari domain service (Wallet, Budget, Goal, Forecast, Calendar Service) — service yang sama yang dipanggil API route biasa. Tidak ada `PrismaClient` yang diimpor di folder `ai/`.
...
**Decision.** Query agregasi kompleks (Forecast computation, Budget realization) memakai kemampuan SQL native (raw query terkontrol via Prisma) alih-alih menarik seluruh baris ke aplikasi lalu mengagregasi di memori (`08. System Design.md` 8.1).
...
Build command menjalankan migrasi database secara otomatis sebelum build aplikasi (`prisma migrate deploy && next build`), memastikan skema database selalu sinkron dengan kode yang di-deploy — tidak ada window waktu di mana kode baru berjalan melawan skema lama.
